...
from data_loader import KITTIDataset  # Updated import
from model import StereoDeepVO        # Updated import
import yaml
import os
...
def test(config):
    ...
    for seq in cfg['sequences']['test']:
        ...
        gt_poses[:, 3, 3] = 1
        gt_trajectory = gt_poses[:, :3, 3]
        
        pred_rel_poses = []
        with torch.no_grad():
            for left_imgs, _ in dataloader:
                left_imgs = left_imgs.to(device)
                pred_pose = model(left_imgs).cpu()
                pred_rel_poses.append(pred_pose.squeeze(0))
        
        pred_rel_poses = torch.cat(pred_rel_poses, dim=0)
...
if __name__ == "__main__":
    ...
    left_imgs = torch.randn(1, 2, 3, 184, 608)
